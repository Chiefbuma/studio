...
---
### e. MySQL / MariaDB Table Creation Scripts (for XAMPP or Docker)
If you are using XAMPP or the provided Docker environment, use these scripts in phpMyAdmin or a similar tool.
...
### g. Local Development with Docker (Recommended)

The recommended way to run the entire local development stack is by using Docker Compose. This will start the Next.js application, a MySQL database, and a database management tool (phpMyAdmin) with a single command.
...
1.  **Start the Docker Containers:**
    Open your terminal in the project root and run the following command. This will build the application image and start all the services in the background.
    ```bash
    docker-compose up --build -d
    ```

2.  **Access the Services:**
    Once the containers are running, you can access the different parts of your development environment:
    *   **Next.js Application**: `http://localhost:3000`
    *   **phpMyAdmin (Database Manager)**: `http://localhost:8080`

3.  **Connecting to the Database with phpMyAdmin:**
    -   When you open phpMyAdmin, use the following credentials on the login screen:
    -   **Server**: `mysql` (This is the service name from `docker-compose.yml`)
    -   **Username**: `root`
    -   **Password**: `secret`
    -   You will see the `WhiskeDelightsDB` database already created.

4.  **Create Tables and Seed Data:**
    -   Inside phpMyAdmin, select the `WhiskeDelightsDB` database from the left sidebar.
    -   Click the `SQL` tab.
    -   Copy and run the **MySQL/MariaDB `CREATE TABLE` scripts** from section `6.e` above.
    -   After the tables are created, go to the `SQL` tab again.
    -   Copy and run the **MySQL/MariaDB Seed Data** scripts from section `6.f` to populate the tables.
...
5.  **Stopping the Environment:**
    ...
    docker-compose down
    ```
    Your database data will be preserved in a Docker volume (`dbdata`).

### h. Connecting Your Backend to the Database

Your backend application (which you will build separately, e.g., using Node.js/Express) will handle all database operations. In that **separate backend project**, you would have an environment file (`.env`) with a connection string.

**When running everything with Docker Compose**, your backend API (if also run as a Docker container on the same `whiskedelights-network`) would use the service name `mysql` as the host:
```
DATABASE_URL="mysql://whiskedelight_user:secret@mysql:3306/WhiskeDelightsDB"
```

**If your backend is running outside of Docker** (e.g., directly on your machine with `node index.js`), it would connect via `localhost`:
```
DATABASE_URL="mysql://whiskedelight_user:secret@localhost:3306/WhiskeDelightsDB"
```
...
- **Docker**: (Optional but Recommended) For running the application and database in a containerized environment.
...
This method runs the Next.js application on your local machine without Docker. You will need to set up your database separately (e.g., using XAMPP).
...
### d. Running with Docker (Recommended)

This is the recommended way to run the entire local development stack, including the application and a database, in a consistent environment.
...
2.  **Access the services:**
    *   **Your Application**: Available at `http://localhost:3000`
    *   **Database Admin (phpMyAdmin)**: Available at `http://localhost:8080`. You can use this to manage your `WhiskeDelightsDB` database.
        *   Server: `mysql`
        *   Username: `root`
        *   Password: `secret`
